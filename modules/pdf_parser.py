...
DEFAULT_GRID_FACTOR_CSV = os.path.join(BASE_DIR, "data", "grid_emission_factors.csv")
...
# ─────────────────────────────────────────
# FUNCTION 10: Extract Narrative Text
# ─────────────────────────────────────────
def extract_narrative_text(pages):
    """
    Extracts all narrative/descriptive text from the PDF for sustainability analysis.
    Concatenates text from all pages, filtering out pure number tables.
    
    Returns narrative text as string for sustainability analysis.
    """
    ...
    for page_num in sorted(pages.keys()):
        ...
        # Filter out pages that are mostly tables/numbers
        # Keep pages with substantial text content
        lines = text.split('\n')
        text_lines = [line for line in lines if len(line.strip()) > 10]
        
        if len(text_lines) > 5:  # At least some substantial content
            # Combine lines but remove excessive whitespace
            cleaned_text = ' '.join(text_lines)
            narrative_parts.append(cleaned_text)
    ...
    if full_narrative:
        print(f"Narrative text extracted: {len(full_narrative)} characters")
    else:
        print("WARNING: No narrative text found in PDF")
...
# ─────────────────────────────────────────
def parse_pdf(pdf_path, csv_path=None):
    """
    ...
    if csv_path is None:
        csv_path = DEFAULT_GRID_FACTOR_CSV
    ...
    # Extract narrative text for sustainability analysis
    narrative_text = extract_narrative_text(pages)
